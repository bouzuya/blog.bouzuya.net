...
Docker を導入するときは、それに関連して必要になるものがたくさんある。一番分かりやすいのがコンテナの管理で、複数のマシンに複数のコンテナを配置すると、管理すべき対象が単純に増えるし、構成は複雑になる。ぼくはとりあえず CoreOS でそれを緩和したいと思っている。Kubernetes が流行っているけど、どうなんだろうね。
